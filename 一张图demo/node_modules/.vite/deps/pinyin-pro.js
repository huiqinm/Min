...
var m = class {
  ...
  buildTrie(n2) {
    for (let h2 of n2) {
      const { zh: n3, priority: i2 } = h2;
      let g2 = this.root;
      for (let h3 = 0; h3 < n3.length; h3++) {
        let i3 = n3.charAt(h3);
        g2.children.has(i3) || g2.children.set(i3, new w()), g2 = g2.children.get(i3);
      }
      g2.isEnd = true, (!g2.pattern || i2 >= g2.pattern.priority) && (g2.pattern = h2);
    }
    this.buildFailPointer();
  }
  rebuildTrie(n2) {
    this.root = new w(), this.buildTrie(n2), this.buildFailPointer();
  }
  buildFailPointer() {
    let n2 = [];
    for (let [h2, i2] of this.root.children)
      i2.fail = this.root, n2.push(i2);
    for (; n2.length > 0; ) {
      let h2 = n2.shift();
      for (let [i2, g2] of h2.children) {
        let u2 = h2.fail;
        for (; null !== u2 && !u2.children.has(i2); )
          u2 = u2.fail;
        g2.fail = null === u2 ? this.root : u2.children.get(i2), n2.push(g2);
      }
    }
  }
  search(n2) {
    let h2 = this.root, i2 = [];
    for (let g2 = 0; g2 < n2.length; g2++) {
      let u2 = n2.charAt(g2);
      for (; null !== h2 && !h2.children.has(u2); )
        h2 = h2.fail;
      if (null === h2)
        h2 = this.root;
      else {
        h2 = h2.children.get(u2), h2.isEnd && i2.push(Object.assign(Object.assign({}, h2.pattern), { index: g2 - h2.pattern.length + 1 }));
        let n3 = h2.fail;
        for (; null !== n3 && n3.isEnd; )
          i2.push(Object.assign(Object.assign({}, n3.pattern), { index: g2 - n3.pattern.length + 1 })), n3 = n3.fail;
      }
    }
  ...
  filter(n2) {
    ...
    let i2 = 0;
    for (let g2 = 0; g2 < n2.length; g2++) {
      const { index: u2, length: s2, priority: z2 } = n2[g2];
      u2 >= i2 ? (h2.push(n2[g2]), i2 = u2 + s2) : z2 > h2[h2.length - 1].priority && (h2[h2.length - 1] = n2[g2], i2 = u2 + s2);
    }
    return h2;
  }
};
...
var k = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g;
function a(n2) {
  return n2.replace(k, "_").length;
}
var Z = {};
function E(n2 = {}) {
  Z = {};
  Object.keys(n2).sort((n3, h3) => a(h3) - a(n3)).forEach((h3) => {
    Z[h3] = n2[h3];
  });
  const h2 = Object.keys(Z).map((n3) => ({ zh: n3, pinyin: Z[n3], priority: 999 + a(n3), length: n3.length }));
  p.rebuildTrie([...h2, ...f]), e.rebuildTrie([...h2, ...r]);
}
var T = /^[\uD800-\uDBFF]$/;
var P = /^[\uDC00-\uDFFF]$/;
var O = (n2) => {
  const h2 = n2.charCodeAt(0), i2 = y[h2];
  return i2 ? i2.split(" ")[0] : n2;
};
var C = (n2) => n2.replace(/(ā|á|ǎ|à)/g, "a").replace(/(ō|ó|ǒ|ò)/g, "o").replace(/(ē|é|ě|è)/g, "e").replace(/(ī|í|ǐ|ì)/g, "i").replace(/(ū|ú|ǔ|ù)/g, "u").replace(/(ǖ|ǘ|ǚ|ǜ)/g, "ü").replace(/(ń|ň|ǹ)/g, "n").replace(/ḿ|m̀/g, "m");
var v = (u2) => {
  const s2 = u2.split(" "), z2 = [], o2 = [];
  for (let u3 of s2)
    for (let s3 of n)
      if (u3.startsWith(s3)) {
        let n2 = u3.slice(s3.length);
        -1 !== h.indexOf(s3) && -1 !== i.indexOf(n2) && (n2 = g[n2]), z2.push(s3), o2.push(n2);
        break;
      }
...
var F = (n2) => {
  const { final: h2 } = v(n2);
  let i2 = "", g2 = "", s2 = "";
  return -1 !== u.indexOf(C(h2)) ? (i2 = h2[0], g2 = h2[1], s2 = h2.slice(2)) : (g2 = h2[0] || "", s2 = h2.slice(1) || ""), { head: i2, body: g2, tail: s2 };
};
var $ = (n2) => {
  const h2 = /(ā|ō|ē|ī|ū|ǖ)/, i2 = /(á|ó|é|í|ú|ǘ|ń|ḿ)/, g2 = /(ǎ|ǒ|ě|ǐ|ǔ|ǚ|ň)/, u2 = /(à|ò|è|ì|ù|ǜ|ǹ|m̀)/, s2 = /(a|o|e|i|u|ü|n)/, z2 = [];
  return n2.split(" ").forEach((n3) => {
    h2.test(n3) ? z2.push("1") : i2.test(n3) ? z2.push("2") : g2.test(n3) ? z2.push("3") : u2.test(n3) ? z2.push("4") : s2.test(n3) ? z2.push("0") : z2.push("");
  }), z2.join(" ");
};
var A = (n2) => {
  const h2 = [];
...
var D = (n2) => "string" == typeof n2 || (console.error("The first param of pinyin is error: " + n2 + ' is not assignable to type "string".'), false);
var N = (n2, h2) => {
  let i2 = h2.nonZh;
  if ("removed" === i2)
    return n2.filter((n3) => n3.isZh);
  if ("consecutive" === i2) {
    for (let h3 = n2.length - 2; h3 >= 0; h3--) {
      const i3 = n2[h3], g2 = n2[h3 + 1];
      i3.isZh || g2.isZh || (i3.origin += g2.origin, i3.result += g2.result, g2.delete = true);
    }
    return n2.filter((n3) => !n3.delete);
  }
  return n2;
};
var B = (n2, h2) => !(1 !== a(n2) || !h2.multiple) && ((n3, h3 = "normal") => {
  const i2 = n3.charCodeAt(0), g2 = Z[n3] || ("surname" === h3 ? s[n3] : "") || y[i2] || "";
  return g2 ? g2.split(" ").map((h4) => ({ origin: n3, result: h4, isZh: true, originPinyin: h4 })) : [{ origin: n3, result: n3, isZh: false, originPinyin: n3 }];
})(n2, h2.mode);
var H = (n2, h2) => {
  switch (h2.pattern) {
    case "pinyin":
    ...
    case "initial":
      n2.forEach((n3) => {
        n3.result = v(n3.result).initial;
      });
      break;
    case "final":
      n2.forEach((n3) => {
        n3.result = v(n3.result).final;
      });
      break;
    case "first":
      n2.forEach((n3) => {
        n3.result = A(n3.result);
      });
      break;
    case "finalHead":
      n2.forEach((n3) => {
        n3.result = F(n3.result).head;
      });
      break;
    case "finalBody":
      n2.forEach((n3) => {
        n3.result = F(n3.result).body;
      });
      break;
    case "finalTail":
      n2.forEach((n3) => {
        n3.result = F(n3.result).tail;
      });
  }
};
var W = (n2, h2) => {
  switch (h2.toneType) {
    ...
    case "none":
      n2.forEach((n3) => {
        n3.result = C(n3.result);
      });
      break;
    case "num":
      n2.forEach((n3) => {
        n3.result = ((n4, h3) => {
          const i2 = C(n4).split(" "), g2 = $(h3).split(" "), u2 = [];
          return i2.forEach((n5, h4) => {
            u2.push(`${n5}${g2[h4]}`);
          }), u2.join(" ");
...
var S = (n2, h2) => {
  h2.v && n2.forEach((n3) => {
    n3.result = n3.result.replace(/ü/g, "v");
  });
};
var M = (n2) => {
  for (let h2 = n2.length - 2; h2 >= 0; h2--) {
    const i2 = n2[h2], g2 = n2[h2 + 1];
    T.test(i2.origin) && P.test(g2.origin) && (i2.origin += g2.origin, i2.result += g2.result, i2.originPinyin = i2.result, g2.delete = true, h2--);
  }
  return n2 = n2.filter((n3) => !n3.delete);
};
var _ = { pattern: "pinyin", toneType: "symbol", type: "string", multiple: false, mode: "normal", removeNonZh: false, nonZh: "spaced", v: false };
function G(n2, h2 = _) {
  if (!D(n2))
    return n2;
  if ("" === n2)
  ...
  let i2 = Array(n2.length);
  return i2 = ((n3, h3, i3) => {
    const g2 = ("surname" === i3 ? e : p).search(n3);
    let u2 = 0;
    for (let i4 = 0; i4 < n3.length; ) {
      const s2 = g2[u2];
      if (s2 && i4 === s2.index) {
        const n4 = s2.pinyin.split(" ");
        let g3 = 0;
        for (let u3 = 0; u3 < s2.length; u3++)
          T.test(s2.zh[u3 - 1]) && P.test(s2.zh[u3]) ? h3[i4 + u3] = { origin: s2.zh[u3], result: "", isZh: true, originPinyin: "" } : (h3[i4 + u3] = { origin: s2.zh[u3], result: n4[g3], isZh: true, originPinyin: n4[g3] }, g3++);
        i4 += s2.length, u2++;
      } else {
        const g3 = n3[i4], u3 = O(g3);
        h3[i4] = { origin: g3, result: u3, isZh: u3 !== g3, originPinyin: u3 }, i4++;
      }
    }
    return h3;
  })(n2, i2, h2.mode || "normal"), i2 = M(i2), i2 = N(i2, h2), B(n2, h2) && (i2 = B(n2, h2)), H(i2, h2), W(i2, h2), S(i2, h2), ((n3, h3, i3) => {
    if (h3.multiple && 1 === a(i3)) {
      let h4 = "";
      n3 = n3.filter((n4) => {
        const i4 = n4.result !== h4;
        return h4 = n4.result, i4;
      });
    }
    return "array" === h3.type ? n3.map((n4) => n4.result) : "all" === h3.type ? n3.map((n4) => {
      const h4 = n4.isZh ? n4.result : "", { initial: i4, final: g2 } = v(h4), { head: u2, body: s2, tail: z2 } = F(h4);
      return { origin: n4.origin, pinyin: h4, initial: i4, final: g2, first: n4.isZh ? A(n4.result) : "", finalHead: u2, finalBody: s2, finalTail: z2, num: Number($(n4.originPinyin)), isZh: n4.isZh };
    }) : n3.map((n4) => n4.result).join(" ");
  })(i2, h2, n2);
}
var I = { precision: "first", continuous: false, space: "ignore", lastPrecision: "start" };
var J = (n2, h2, i2) => {
  "any" === (null == i2 ? void 0 : i2.precision) && (i2.lastPrecision = "any");
  const g2 = Object.assign(Object.assign({}, I), i2 || {});
  "ignore" === g2.space && (h2 = h2.replace(/\s/g, ""));
  return "any" === (null == i2 ? void 0 : i2.precision) ? L(n2, h2, g2) : Q(n2, h2, g2);
};
var K = (n2, h2) => {
  let i2 = 0;
  for (let g2 = 0; g2 < n2.length; g2++)
...
var L = (n2, h2, i2) => {
  ...
  for (let u2 = 0; u2 < n2.length; u2++) {
    if ("ignore" === i2.space && " " === n2[u2]) {
      g2.push(u2);
      continue;
    }
    if (n2[u2] === h2[0]) {
      h2 = h2.slice(1), g2.push(u2);
      continue;
    }
    const s2 = G(n2[u2], { toneType: "none", multiple: true, type: "array" });
    let z2 = 0;
    if (s2.forEach((n3) => {
      const i3 = K(n3, h2);
      i3 > z2 && (z2 = i3);
    }), z2 && (h2 = h2.slice(z2), g2.push(u2)), !h2)
      break;
  }
  if (h2)
    return null;
  if (i2.continuous) {
    const n3 = g2;
    if (g2.some((h3, i3) => i3 > 0 && h3 !== n3[i3 - 1] + 1))
      return null;
  }
  return "ignore" === i2.space && (g2 = g2.filter((h3) => " " !== n2[h3])), g2.length ? g2 : null;
};
var Q = (n2, h2, i2) => {
  const g2 = n2.split(""), u2 = Array(g2.length + 1);
  for (let n3 = 0; n3 < u2.length; n3++)
    u2[n3] = Array(h2.length + 1), u2[n3][0] = [];
  for (let n3 = 0; n3 < u2[0].length; n3++)
    u2[0][n3] = [];
  for (let g3 = 1; g3 < u2.length; g3++) {
    if (!i2.continuous || "ignore" == i2.space && " " === n2[g3 - 1])
      for (let n3 = 1; n3 <= h2.length; n3++)
        u2[g3][n3 - 1] = u2[g3 - 1][n3 - 1];
    for (let s2 = 1; s2 <= h2.length; s2++)
      if (u2[g3 - 1][s2 - 1] && (1 === s2 || u2[g3 - 1][s2 - 1].length)) {
        const z2 = G(n2[g3 - 1], { type: "array", toneType: "none", multiple: true });
        if (n2[g3 - 1] === h2[s2 - 1]) {
          const n3 = [...u2[g3 - 1][s2 - 1], g3 - 1];
          if ((!u2[g3][s2] || n3.length > u2[g3][s2].length) && (u2[g3][s2] = n3), s2 === h2.length)
            return u2[g3][s2];
        }
        if (h2.length - s2 <= 6) {
          if (z2.some((n3) => "any" === i2.lastPrecision ? n3.includes(h2.slice(s2 - 1, h2.length)) : "start" === i2.lastPrecision ? n3.startsWith(h2.slice(s2 - 1, h2.length)) : "first" === i2.lastPrecision ? n3[0] === h2.slice(s2 - 1, h2.length) : "every" === i2.lastPrecision && n3 === h2.slice(s2 - 1, h2.length)))
            return [...u2[g3 - 1][s2 - 1], g3 - 1];
        }
        const o2 = i2.precision;
        if ("start" === o2 && z2.forEach((n3) => {
          let i3 = s2;
          const z3 = [...u2[g3 - 1][s2 - 1], g3 - 1];
          for (; i3 <= h2.length && n3.startsWith(h2.slice(s2 - 1, i3)); )
            (!u2[g3][i3] || z3.length > u2[g3][i3].length) && (u2[g3][i3] = z3), i3++;
        }), "first" === o2 && z2.some((n3) => n3[0] === h2[s2 - 1])) {
          const n3 = [...u2[g3 - 1][s2 - 1], g3 - 1];
          (!u2[g3][s2] || n3.length > u2[g3][s2].length) && (u2[g3][s2] = n3);
        }
        const y2 = z2.find((n3) => n3 === h2.slice(s2 - 1, s2 - 1 + n3.length));
        if (y2) {
          const n3 = [...u2[g3 - 1][s2 - 1], g3 - 1], h3 = s2 - 1 + y2.length;
          (!u2[g3][h3] || n3.length > u2[g3][h3].length) && (u2[g3][h3] = n3);
        }
      }
  }
  return null;
};
var R = { resultClass: "py-result-item", chineseClass: "py-chinese-item", pinyinClass: "py-pinyin-item", nonChineseClass: "py-non-chinese-item", wrapNonChinese: false, toneType: "symbol" };
var U = (n2, h2) => {
  const i2 = Object.assign(Object.assign({}, R), h2 || {});
  return G(n2, { type: "all", toneType: i2.toneType }).map((n3) => {
    if (n3.isZh) {
      const h3 = i2.resultClass, g2 = i2.chineseClass, u2 = i2.pinyinClass;
      return `<span class="${h3}"><ruby><span class="${g2}">${n3.origin}</span><rp>(</rp><rt class="${u2}">${n3.pinyin}</rt><rp>)</rp></ruby></span>`;
    }
    if (i2.wrapNonChinese) {
      return `<span class="${i2.nonChineseClass}">${n3.origin}</span>`;
    }
    return n3.origin;
  }).join("");
};
var V = { pattern: "pinyin", toneType: "symbol", type: "string", v: false, nonZh: "spaced" };
function X(n2, h2 = V) {
  if (!D(n2))
    return [];
  if ("" === n2)
    return [];
  "all" === h2.type && (h2.pattern = "pinyin"), "num" === h2.pattern && (h2.toneType = "none");
  let i2 = Y(n2);
  i2 = M(i2), i2 = N(i2, h2);
  let g2 = nn(i2);
  g2.forEach((n3) => {
  ...
  return g2.map((n3) => hn(n3, h2));
}
var Y = (n2) => n2.split("").map((n3) => {
  const h2 = n3.charCodeAt(0), i2 = y[h2] || "";
  return { origin: n3, result: i2, isZh: !!i2, originPinyin: i2 };
});
var nn = (n2) => n2.map((n3) => n3.isZh ? n3.result.split(" ").map((h2) => ({ origin: n3.origin, result: h2, isZh: true, originPinyin: h2 })) : [n3]);
var hn = (n2, h2) => "array" === h2.type ? Array.from(new Set(n2.map((n3) => n3.result))) : "all" === h2.type ? n2.map((n3) => {
  const h3 = n3.isZh ? n3.result : "", { initial: i2, final: g2 } = v(h3), { head: u2, body: s2, tail: z2 } = F(h3);
  return { origin: n3.origin, pinyin: h3, initial: i2, final: g2, first: n3.isZh ? A(n3.result) : "", finalHead: u2, finalBody: s2, finalTail: z2, num: Number($(n3.originPinyin)), isZh: n3.isZh };
}) : Array.from(new Set(n2.map((n3) => n3.result))).join(" ");
export {
  E as customPinyin,
  U as html,
  J as match,
